...
videoFileName='videorecord.avi'
videoDimension=(frameWidth,frameHeight)
videorecord=cv2.VideoWriter(videoFileName,fourccCode,frameRate,videoDimension)


#for saving audio
audio = pyaudio.PyAudio()

stream = audio.open(format=pyaudio.paInt16, channels=1,rate=16000,input = True,frames_per_buffer=3200)
...
while True:
    ...
    k= cv2.waitKey(1)
    if cv2.getWindowProperty('Air_Canvas',cv2.WND_PROP_VISIBLE) <1:
        break
...
sound_file = wave.open("audiorecord.wav","wb")
